...
public class KickUserForPoints implements UserContextInteraction {
    @Override
        ...
        if(PlayerInfos.isExist(event.getMember().getId(), "discord_id", "users")){
            if(PunkteSystem.getPoints(event.getMember().getId()) >= 1000){
                if(event.getTargetMember().getVoiceState().inAudioChannel()) {
                    ...
                    PunkteSystem.upload(event.getMember().getId(), 1000, 0, event.getTargetMember().getUser().getName() + " gekicked.");

                    event.getGuild().kickVoiceMember(event.getTargetMember()).queue();

                    LogSystem.logGeneral(event.getMember().getId(), "User hat die Verbindung von " + event.getTargetMember().getUser().getName() + " getrennt.", event.getMember().getUser().getAsTag());

                    EmbedBuilder builder = new EmbedBuilder();
                    builder.setColor(Color.decode("#2ecc71"));
                    builder.setThumbnail(BotInfos.getBotInfos("logo_url"));
                    builder.setDescription("Du hast den User (" + event.getTargetMember().getUser().getName() + ") gekicked");
                    builder.setFooter("PS: Keiner weiß das du es warst.");
                    builder.setTitle("Erfolgreich die Verbindung getrennt!");
