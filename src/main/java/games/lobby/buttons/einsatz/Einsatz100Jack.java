...
public class Einsatz100Jack implements ServerButton {
    @Override
    public void performCommand(ButtonInteractionEvent event) {
        Member m = Main.INSTANCE.getGuild().getMemberById(event.getId().replace("100e", ""));
        ...
        for (Player p : l.getPlayer()) {
            if(p.getM() == event.getMember()){
                ...
                if(p.getEinsatz() == 0) {
                    ...
                    builder.setDescription("Du hast deinen Einsatz auf 100 Points gesetzt.");
